...
class MeetEventSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MeetEvent
        fields = ('id', 'swim_meet', 'num_event', 'name', 'group', 'group_name', 'event_type', 'event_type_name' )
    ...
    def validate_unique_constrains(self, data):
        ...
        if MeetEvent.objects.filter(swim_meet=swim_meet, num_event=num_event).exists():
            raise serializers.ValidationError({'num_event': 'Event with this number already exists for the swim meet.'})
