...
from api.models import Group, Heat, MeetEvent, SwimMeet
...
def create_excel_file_for_all_events_results(swim_meet_details, results_by_event):
    ...
    results_worksheet = workbook.active
    results_worksheet.title = "General Results"
    results_headers = ["Rank", "Athlete", "Heat Time"]
    current_row = 1
    ...
    for event in results_by_event:
        ...
        if event["results"]:
            ...
            results_worksheet.cell(row=current_row, column=1,
                                   value="This event has no results yet.")
            current_row += 2
...
class DownloadEventResults(APIView):
    ...
    def post(self, request, event_id):
        ...
        general_results = get_queryset(event_instance)
        ...
        if serializer.is_valid():
            ...
            for group_id in group_ids:
                group_results = get_queryset(event_instance, group_id)
                if group_results:
...
class DownloadEventResultsForSwimMeet(APIView):
    ...
    def post(self, request, meet_id):
        ...
        for event in events_instance:
            ...
            general_results = get_queryset(event)

            event_data.append(
                {
                    "event_name": event_name,
                    "results": general_results if general_results else None,
                }
            )

        # Generate Excel file
        file = create_excel_file_for_all_events_results(swim_meet_details,
                                                        event_data)
        file_name = f"{swim_meet_instance.name}_Results.xlsx"
...
def get_queryset(event_instance, group_id=None):
    ...
    if any(result.heat_time is None for result in results):
